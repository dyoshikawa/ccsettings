import { readSettings, getSettingsPath } from '../../core/settings.js';
...
export async function showCommand(): Promise<void> {
  try {
    ...
    if (settings.permissions) {
      ...
      if (settings.permissions.additionalDirectories && settings.permissions.additionalDirectories.length > 0) {
    ...
    if (settings.hooks && Object.keys(settings.hooks).length > 0) {
      ...
      Object.entries(settings.hooks).forEach(([hookType, hooks]) => {
        ...
        Object.entries(hooks as Record<string, string>).forEach(([tool, command]) => {
